...
/**
 * Mirrors SillyTavern's native World Info activation-state notation.
 *
 * @param {Record<string, any>} entry
 * @returns {{type: 'constant'|'keyword'|'vectorized', label: 'Constant'|'Keyword'|'Vectorized', icon: string}}
 */
export function getEntryActivationPresentation(entry) {
    const strategy = entryStrategy(entry).toLowerCase();
    if (entry.constant === true || strategy === 'constant') {
        return { type: 'constant', label: 'Constant', icon: '🔵' };
    }
    if (entry.vectorized === true || strategy === 'vectorized') {
        return { type: 'vectorized', label: 'Vectorized', icon: '🔗' };
    }
    return { type: 'keyword', label: 'Keyword', icon: '🟢' };
}
...
function buildEntryRow(entry, promptIndex = null) {
    ...
    main.append(createElement('span', 'wim-entry-title', entryTitle(entry)));
    if (promptIndex !== null) {
        main.append(createElement('span', 'wim-entry-world', entryWorld(entry)));
    }

    const metadata = createElement('span', 'wim-entry-metadata');
    const activation = getEntryActivationPresentation(entry);
    const activationMarker = createElement(
        'span',
        `wim-entry-activation wim-entry-activation-${activation.type}`,
    );
    activationMarker.title = activation.label;
    activationMarker.setAttribute('aria-label', activation.label);
    const activationIcon = createElement('span', 'wim-entry-activation-icon', activation.icon);
    activationIcon.setAttribute('aria-hidden', 'true');
    activationMarker.append(activationIcon);

    const sticky = entrySticky(entry);
    if (sticky !== undefined && sticky !== null && sticky !== '' && Number(sticky) > 0) {
        activationMarker.append(createElement('span', 'wim-entry-sticky', `(${sticky})`));
    }
